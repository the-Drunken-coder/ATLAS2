...
var require_validate = __commonJS({
  "node_modules/.pnpm/ajv@8.20.0/node_modules/ajv/dist/compile/validate/index.js"(exports) {
    ...
    function keywordCode2(it, keyword, def, ruleType) {
...
function instancePathToField(instancePath) {
  ...
  const decoded = instancePath.split("/").slice(1).map((segment) => segment.replace(/~1/g, "/").replace(/~0/g, "~"));
  return `json.${decoded.join(".")}`;
}
function keywordCode(error) {
...
var MAX_JSON_BLOB_SIZE = 64 * 1024;
var MAX_JSON_DEPTH = 16;
var MAX_JSON_FIELDS = 500;
var MAX_JSON_KEY_LENGTH = 100;
var MAX_CUSTOM_BLOB_SIZE = 16 * 1024;
var MAX_CUSTOM_DEPTH = 8;
var MAX_CUSTOM_FIELDS = 100;
var MAX_CUSTOM_KEY_LENGTH = 100;
var COMMAND_CATALOG_OBJECT_ID = "command_catalog";
var promotedFields = /* @__PURE__ */ new Set([
  "entity_id",
  "object_id",
  "task_id",
  "observation_id",
...
var topLevelReservedForCustom = /* @__PURE__ */ new Set([
  "components",
  "extra",
  "description",
  "created_by",
...
var topLevelAllowedBySchema = {
  entity: /* @__PURE__ */ new Set(["components", "extra"]),
  task: /* @__PURE__ */ new Set(["description", "created_by", "components", "extra"]),
  observation: /* @__PURE__ */ new Set(["state", "latest_sighting", "sightings_object_id", "extra"])
};
...
function add(out, field, code, message) {
  out.push({ field, code, message });
}
function validateLimits(value, field, out, limits) {
  const size = Buffer2.byteLength(JSON.stringify(value));
  if (size > limits.maxSize) {
    add(out, field, "TOO_LARGE", `must be ${limits.maxSize} bytes or fewer`);
  }
  let fields = 0;
  const walk = (current, currentField, depth) => {
...
function validateCustomSection(path, value, out) {
...
function validateTopLevelKeys(root, allowed, out) {
...
function validateEntityRules(root, context, out) {
  ...
  if (!isPlainObject(componentsValue)) {
    if (componentsValue === void 0) {
      add(out, "json.components", "REQUIRED", "is required");
    }
    return;
  }
  const known = /* @__PURE__ */ new Set([...matrix.required, ...matrix.optional]);
  ...
  const sensors = isPlainObject(componentsValue.sensor_refs) ? componentsValue.sensor_refs.sensors : void 0;
...
function validateObjectRules(root, context, out) {
  ...
  const allowed = /* @__PURE__ */ new Set(["extra"]);
  if (objectType === "log") {
...
function applyAtlasRules(schemaName, root, rawJson, context) {
  ...
  if (Buffer2.byteLength(rawJson) > MAX_JSON_BLOB_SIZE) {
...
var entity_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://atlas.local/schemas/entity.schema.json",
  title: "Atlas entity JSON",
  $defs: {
    customSection: {
      type: "object"
    },
    ...
    fusionSummary: {
      ...
      properties: {
        ...
        provenance_object_id: { type: "string" }
      }
    }
  },
  type: "object",
  properties: {
    components: {
      type: "object",
      properties: {
        supported_commands: { $ref: "#/$defs/supportedCommands" },
        ...
        sensor_refs: { $ref: "#/$defs/sensorRefs" },
        fusion_summary: { $ref: "#/$defs/fusionSummary" }
      },
      patternProperties: {
        "^custom_": { $ref: "#/$defs/customSection" }
      },
      additionalProperties: true
    },
    extra: { type: "object" }
...
var object_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://atlas.local/schemas/object.schema.json",
  title: "Atlas object JSON",
  $defs: {
    parameterSchema: {
      type: "object",
      required: ["type"],
      ...
      properties: {
        ...
        properties: {
          type: "object",
          additionalProperties: { $ref: "#/$defs/parameterSchema" }
        },
        required: {
          type: "array",
          items: { type: "string", minLength: 1 },
    ...
    commands: {
      ...
      additionalProperties: {
        ...
        properties: {
          ...
          parameters_schema: { $ref: "#/$defs/parameterSchema" }
        },
        additionalProperties: true
      }
    },
...
var task_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://atlas.local/schemas/task.schema.json",
  title: "Atlas task JSON",
  $defs: {
    customSection: {
      type: "object"
    }
...
var observation_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://atlas.local/schemas/observation.schema.json",
  title: "Atlas observation JSON",
  $defs: {
    customSection: {
      type: "object"
    }
  ...
  properties: {
    ...
    latest_sighting: {
      type: "object",
      additionalProperties: false,
      required: ["observed_at", "kind", "data"],
      properties: {
        observed_at: { type: "string", format: "date-time" },
    ...
    sightings_object_id: { type: "string" },
    extra: { type: "object" }
  },
  patternProperties: {
    "^custom_": { $ref: "#/$defs/customSection" }
...
var command_catalog_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://atlas.local/schemas/command-catalog.schema.json",
  title: "Atlas command catalog payload",
  $defs: {
    parameterSchema: {
      type: "object",
      required: ["type"],
...
var change_event_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://atlas.local/schemas/change-event.schema.json",
  title: "Atlas change event",
  type: "object",
  additionalProperties: false,
  required: ["resource_kind", "operation", "resource_id", "occurred_at"],
  properties: {
    resource_kind: {
      type: "string",
      enum: ["entity", "object", "task", "observation"]
    },
    operation: {
      type: "string",
      enum: ["create", "update", "delete", "manifest_sync"]
    },
...
var validation_error_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://atlas.local/schemas/validation-error.schema.json",
  title: "Atlas validation error",
  type: "object",
  additionalProperties: false,
...
var schemas = {
  ...
  "object": object_schema_default,
  "task": task_schema_default,
  "observation": observation_schema_default,
  "command-catalog": command_catalog_schema_default,
  "change-event": change_event_schema_default,
  "validation-error": validation_error_schema_default
};
...
function stableValue(value) {
  ...
  const entries = Object.entries(value).sort(([left], [right]) => left.localeCompare(right)).map(([key, child]) => [key, stableValue(child)]);
...
function parseRoot(raw) {
  try {
    ...
    return { ok: true, value: parsed, normalized: "", errors: [] };
  } catch {
    return { ok: false, errors: [{ field: "json", code: "INVALID_JSON", message: "must be valid JSON" }] };
  }
}
function validateJson(schemaName, rawJson, context = {}) {
...
function parseRequest(argv) {
  ...
  return {
    ...
    context: {
      ...
      entityType: args.get("entity-type"),
      objectType: args.get("object-type"),
      objectId: args.get("object-id") ?? void 0
    },
    json
  };
}
var request = parseRequest(process.argv.slice(2));
var result = validateJson(request.schema, request.json, request.context);
