...
1. Uses the same JSON Schema documents as `atlas-protocol/source/schemas/`, or
   a deterministic bundle derived from them (for example, the Go package embeds
   `atlas-protocol/packages/go/generated/schema_bundle.json`, which is kept
   byte-identical to `atlas-protocol/generated/schema-bundle.json` and
...
2. Implements the same **custom rules** documented in contracts and
   ...
   manifest fields, catalog `parameter_schema` typo detection, GeoJSON topology
   checks, change-event snapshot rules, and any other rules the reference
   validator applies outside JSON Schema).
...
schema bundle produced next to the TypeScript build).
