...
Vertical Slice 2 integrates Atlas Protocol validation into the function layer so
Core validates Atlas-shaped JSON before persistence and applies Core-owned
runtime checks that require stored state.
...
- the Core side already has stores, functions, validation, and runtime checks
