...
func TestObjectFunctions_UpsertObjectRestoresExistingMetadataOnFolderExistsFailure(t *testing.T) {
	original := &model.Object{
		ObjectID:  "obj_001",
		Type:      model.ObjectTypeLog,
		OwnerType: model.OwnerTypeSystem,
	...
	storage := fakeObjectStorage{
		existsFn: func(string) (bool, error) { return false, fmt.Errorf("inspect failed") },
	}
	f := NewObjectFunctions(pg, storage, fakeIdempotencyStore{}, testLogger())
