...
func (c *ObjectGatewayClient) WriteFile(ctx context.Context, objectID, filename string, data []byte) (functionpkg.MutationResult, error) {
	resp, err := c.client.WriteObjectFile(ctx, &sharedv1.WriteObjectFileRequest{ObjectId: objectID, Filename: filename, Data: data})
	if err != nil {
		return functionpkg.MutationResult{}, rpcerrors.FromStatus(err)
	}
	var manifest *model.ObjectManifest
	if resp.GetManifest() != nil {
		converted, convErr := pbconv.ManifestFromProto(resp.GetManifest())
		if convErr != nil {
			return functionpkg.MutationResult{}, convErr
		}
		manifest = converted
	}
	return functionpkg.MutationResult{
		Manifest:        manifest,
		ManifestCurrent: resp.GetManifestCurrent(),
		SyncError:       resp.GetManifestSyncError(),
	}, nil
}
func (c *ObjectGatewayClient) AppendFile(ctx context.Context, objectID, filename string, data []byte) (functionpkg.MutationResult, error) {
	resp, err := c.client.AppendObjectFile(ctx, &sharedv1.WriteObjectFileRequest{ObjectId: objectID, Filename: filename, Data: data})
	if err != nil {
		return functionpkg.MutationResult{}, rpcerrors.FromStatus(err)
	}
	var manifest *model.ObjectManifest
...
func (c *ObjectGatewayClient) DeleteFile(ctx context.Context, objectID, filename string) (functionpkg.MutationResult, error) {
	resp, err := c.client.DeleteObjectFile(ctx, &sharedv1.ReadObjectFileRequest{ObjectId: objectID, Filename: filename})
	if err != nil {
		return functionpkg.MutationResult{}, rpcerrors.FromStatus(err)
	}
	var manifest *model.ObjectManifest
