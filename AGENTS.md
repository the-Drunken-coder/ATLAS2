...
- **Observation history dedup index**: `historyContainsEventID` is read-only (no index writes during lookup). It checks (1) `extra.event_id_index` when `event_id_index_complete` is true, (2) the partial extra index, (3) `event_ids.ndjson` in memory, then (4) a scan of `history.ndjson` without persisting. Call `bootstrapHistoryEventIDIndex` to rebuild/persist the index from sidecar or history. `appendHistoryEvent` treats `history.ndjson` as authoritative; sidecar/index maintenance failures are logged but do not fail the append. Ingest appends both `history.ndjson` and `event_ids.ndjson`. Do not grow `event_id_index` unbounded without a retention plan—it truncates and falls back to the sidecar.
