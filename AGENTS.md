...
- **Schema without migrations**: the project avoids migration frameworks; schema setup for Postgres lives in application code (`atlas-core/internal/adapters/postgres`). Changing persistence shape means updating that code and any callers/tests—do not add SQL migration files to satisfy the same change.
- **Command catalog object type**: Vertical Slice 2 treats the command catalog as the `document` object with `object_id = command_catalog`; `command_catalog` is not a separate object type for new writes.
- **Postgres-backed tests**: packages under `atlas-core/internal/adapters/postgres` and integration-style tests in `atlas-core/internal/service` expect a reachable Postgres instance. They use `ATLAS_TEST_POSTGRES_*` env vars when set; otherwise defaults target `localhost:5432` and database `atlas_core_test`. If the server is down, tests **skip** rather than fail hard (after connection attempt).
- **Test DB safety**: `atlas-core/internal/adapters/postgres` test helpers refuse to run destructive cleanup unless the configured database name ends with `_test` or `ATLAS_ALLOW_DB_CLEANUP=true`. Do not point tests at a production database name.
