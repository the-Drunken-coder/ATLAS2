...
- **Observation history dedup index**: `historyContainsEventID` checks (1) `extra.event_id_index` when `event_id_index_complete` is true, (2) the partial extra index, (3) append-only `event_ids.ndjson` on the history object, then (4) a one-time legacy rebuild from `history.ndjson` when the sidecar is missing. Ingest appends both `history.ndjson` and `event_ids.ndjson`. Do not grow `event_id_index` unbounded without a retention plan—it truncates and falls back to the sidecar.
