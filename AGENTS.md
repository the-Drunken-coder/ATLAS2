...
- **Observation history dedup index**: `historyContainsEventID` caches event IDs in `observation_history` object `extra.event_id_index` (see `observation_history.go`). The first lookup per object may still read all of `history.ndjson` to bootstrap; after that, dedup is O(1) without re-reading the file. Very large histories may eventually need a separate index file or ranged reads—do not grow `event_id_index` unbounded without a retention plan.
